...
class KfoldDataloader(pl.LightningDataModule):
    def __init__(self, model_name, batch_size, shuffle, train_path, dev_path, test_path, predict_path, k:int=1, split_seed:int=42, num_splits:int=10):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(model_name, max_length=72)
...
def sweep_train(config=None):
    ...
    early_stop_callback = EarlyStopping(monitor='val_pearson', mode='max', patience=3, min_delta=0.00, verbose=False)
    ...
    for k in range(num_folds):
        ...
        trainer = pl.Trainer(gpus=1, max_epochs=args.max_epoch, logger=wandb_logger, log_every_n_steps=1, callbacks=[early_stop_callback, checkpoint_callback])
        trainer.fit(model=model, datamodule=dataloader)
        trainer.test(model=model, datamodule=dataloader)
    
    torch.save(model, 'model_1101_fold_10.pt')
