...
if __name__ == "__main__":
    ...
    # 3. Kr(train&dev) + Ko(train&dev) + Kr(train만-ep48.ckpt) + Ko(train만-ep43.ckpt)

    model = torch.load("KrELECTRA_ep48_bs16_train&dev.pt")
    model2 = torch.load("KoELECTRA_ep43_bs16_train&dev.pt")
    model3 = Model.load_from_checkpoint("Kr_epoch48-val_pearson0.9335.ckpt")
    model4 = Model.load_from_checkpoint("Ko_epoch43-val_pearson0.9239.ckpt")
    # test checkpoint
    ...
    predictions3 = trainer.predict(model=model3, datamodule=dataloader)
    predictions4 = trainer.predict(model=model4, datamodule=dataloader2)

    final_predictions = [
        (predictions[i] + predictions2[i]+ predictions3[i]+ predictions4[i]) / 4 for i in range(len(predictions))
    ]
    final_predictions = list(round(float(i), 1) for i in torch.cat(final_predictions))

    output = pd.read_csv("../../data/sample_submission.csv")
    output["target"] = final_predictions
    output.to_csv("3_Kr(train+dev)_Ko(train+dev)_Kr(train)_Ko(train)_ensemble.csv", index=False)
