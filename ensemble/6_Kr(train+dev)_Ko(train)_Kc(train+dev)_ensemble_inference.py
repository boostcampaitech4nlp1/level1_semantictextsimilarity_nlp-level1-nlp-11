...
if __name__ == "__main__":
    ...
    # 6_Kr(train+dev)_Ko(train)_Kc(train+dev)_ensemble

    model = torch.load("KrELECTRA_ep48_bs16_train&dev.pt")
    model2 = Model.load_from_checkpoint("Ko_epoch43-val_pearson0.9239.ckpt")
    model3 = torch.load("KcELECTRA_train&dev.pt")
    ...
    predictions3 = trainer.predict(model=model3, datamodule=dataloader3)
    

    final_predictions = [
        (predictions[i] + predictions2[i]+ predictions3[i]) / 3 for i in range(len(predictions))
    ]
    final_predictions = list(round(float(i), 1) for i in torch.cat(final_predictions))

    output = pd.read_csv("../../data/sample_submission.csv")
    output["target"] = final_predictions
    output.to_csv("6_Kr(train+dev)_Ko(train)_Kc(train+dev)_ensemble.csv", index=False)
