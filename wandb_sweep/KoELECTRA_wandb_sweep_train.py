...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr, optimizer, lr_scheduler):
        super().__init__()
...
def wandb_sweep(config=None):
    ...
    model = Model(
        args.model_name, config.learning_rate, config.optimizer, config.lr_scheduler
    )
    wandb_logger = WandbLogger(project="dongchan-sweep-tuning")
    ...
    # torch.save(model, "koelectra_wandb_sweep.pt")
...
if __name__ == "__main__":
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "optimizer": {"values": ["AdamW", "AdamP"]},
            "lr_scheduler": {"values": ["cos", "exp"]},
            "batch_size": {"values": [16, 32]},
        },
    }
    sweep_id = wandb.sweep(
        sweep_config, entity="nlp-11", project="dongchan-sweep-tuning"
    )
    wandb.agent(sweep_id=sweep_id, function=wandb_sweep, count=50)
